      ...
      <div className="bg-white rounded-xl border border-slate-200/80 p-4 shadow-xs space-y-3">
        <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-9 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs transition-colors focus:outline-hidden focus:bg-white focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600"
            />
            {search && (
              <button
                onClick={() => setSearch('')}
          ...
          <div className="md:hidden flex items-center gap-2">
            ...
            <select
              ...
              className="w-full text-xs font-medium bg-slate-50 border border-slate-200 rounded-lg py-2 px-3 focus:outline-hidden focus:border-emerald-600"
            >
              <option value="all">Tüm Durumlar ({vehicles.length})</option>
              {STATUS_LIST.map((s) => (
                <option key={s.key} value={s.key}>
                  {s.label} ({vehicles.filter((v) => v.currentStatus === s.key).length})
                </option>
              ))}
            </select>
          </div>
        ...
        <div className="hidden md:flex items-center gap-1.5 flex-wrap pt-1 border-t border-slate-100">
          ...
          {STATUS_LIST.map((status) => {
            const count = vehicles.filter((v) => v.currentStatus === status.key).length;
        ...
        {filteredVehicles.length === 0 ? (
          <div className="p-8">
            <EmptyState
              ...
              actionText={search || statusFilter !== 'all' ? 'Filtreleri Temizle' : 'Yeni Araç Ekle'}
              onAction={
                search || statusFilter !== 'all'
                  ? () => {
                      setSearch('');
